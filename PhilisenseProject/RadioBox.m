...
-(id)initwithDelegate:(id)delegate{
    ...
    if (self) {
        ...
        [self addTarget:self action:@selector(RadioboxBtnChecked) forControlEvents:UIControlEventTouchUpInside];
        [self addTarget:self action:@selector(RadioboxBtnPressed) forControlEvents:UIControlEventTouchDown];
    }
    return self;
}
...
- (void)setChecked:(BOOL)checked{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(didselectedCheckBox:checked:)]) {
        [_delegate didselectedRadioBox:self checked:self.selected];
    }
}

- (IBAction)RadioboxBtnChecked {
    self.selected = !self.selected;
    _checked = self.selected;
    if (_delegate && [_delegate respondsToSelector:@selector(didselectedRadioBox:checked:)]) {
        [_delegate didselectedRadioBox:self checked:self.selected];
    }
}
- (IBAction)RadioboxBtnPressed {
